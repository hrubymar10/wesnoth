...
void title_screen::pre_show(window& win)
{
    ...
    if (res.x >= 800 && res.y >=600) {
        ...
        image* logo_bg_tiny = find_widget<image>(&win, "logo-bg-tiny", false, false);
        if(logo_bg_tiny != nullptr) {
            logo_bg_tiny->set_image(game_config::images::game_logo_background_tiny);
        }
        image* logo_tiny = find_widget<image>(&win, "logo-tiny", false, false);
        if(logo_tiny != nullptr) {
            logo_tiny->set_image(game_config::images::game_logo_tiny);
        }
    }
